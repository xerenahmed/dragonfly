...
func (w *World) Biome(pos cube.Pos) Biome {
	...
	b, ok := BiomeByID(id)
	if !ok {
		w.conf.ErrorLog.Errorf("could not find biome by ID %v", id)
	}
	return b
}
...
func (w *World) Liquid(pos cube.Pos) (Liquid, bool) {
	...
	if !ok {
		w.conf.ErrorLog.Errorf("failed getting liquid: cannot get block by runtime ID %v", id)
		return nil, false
	}
...
func (w *World) removeLiquidOnLayer(c *chunk.Chunk, x uint8, y int16, z, layer uint8) (bool, bool) {
	...
	b, ok := BlockByRuntimeID(id)
	if !ok {
		w.conf.ErrorLog.Errorf("failed removing liquids: cannot get block by runtime ID %v", id)
		return false, false
	}
	if _, ok := b.(Liquid); ok {
...
func (w *World) close() {
	...
	w.conf.ErrorLog.Debugf("Saving chunks in memory to disk...")
	...
	if !w.conf.ReadOnly {
		w.conf.ErrorLog.Debugf("Updating level.dat values...")

		w.provider().SaveSettings(w.set)
	}

	w.conf.ErrorLog.Debugf("Closing provider...")
	if err := w.provider().Close(); err != nil {
		w.conf.ErrorLog.Errorf("error closing world provider: %v", err)
	}
}
...
func (w *World) chunk(pos ChunkPos) *chunkData {
	...
	if !ok {
		...
		if err != nil {
			w.chunkMu.Unlock()
			w.conf.ErrorLog.Errorf("load chunk: failed loading %v: %v\n", pos, err)
			return c
		}
...
func (w *World) loadIntoBlocks(c *chunkData, blockEntityData []map[string]any) {
	...
	for _, data := range blockEntityData {
		...
		b, ok := BlockByRuntimeID(id)
		if !ok {
			w.conf.ErrorLog.Errorf("error loading block entity data: could not find block state by runtime ID %v", id)
			continue
		}
...
func (w *World) saveChunk(pos ChunkPos, c *chunkData) {
	...
	if !w.conf.ReadOnly {
		...
		if err := w.provider().SaveChunk(pos, c.Chunk, w.conf.Dim); err != nil {
			w.conf.ErrorLog.Errorf("error saving chunk %v to provider: %v", pos, err)
		}
		...
		if err := w.provider().SaveEntities(pos, s, w.conf.Dim); err != nil {
			w.conf.ErrorLog.Errorf("error saving entities in chunk %v to provider: %v", pos, err)
		}
		if err := w.provider().SaveBlockNBT(pos, m, w.conf.Dim); err != nil {
			w.conf.ErrorLog.Errorf("error saving block NBT in chunk %v to provider: %v", pos, err)
		}
	}
	ent := c.entities
	c.entities = nil
